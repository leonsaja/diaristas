...
from administracao.service import servico_service
from api.service.cidades_atendimento_service import verificar_disponibilidade_cidade,buscar_cidade_ibge
...
class DiariaSerializer(serializers.ModelSerializer):
    ...
    def validate_preco(self,preco):
        ...
        raise serializers.ValidationError('Valores não correspodem')
    ...
    def validate_data_atendimento(self,data_atendimento):
        if data_atendimento.hour < 6:
            raise serializers.ValidationError('Horario de inicio não pode ser menor que 6')
